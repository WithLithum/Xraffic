...
namespace Traffic_Policer.Ambientevents
{
    ...
    {
        public RevEngineWhenStationary(Ped Driver, bool createBlip, bool showMessage) : base(Driver, createBlip, showMessage, "Creating RevEngineWhenStationary event")
        {
            MainLogic();
        }
            ...
            AmbientEventMainFiber = GameFiber.StartNew(delegate
            {
                try
                {
                    ...
                    while (eventRunning)
                    {
                        ...
                        if (car.Speed < 2f)
                        {
                            driver.Tasks.PerformDrivingManeuver(VehicleManeuver.RevEngine);
